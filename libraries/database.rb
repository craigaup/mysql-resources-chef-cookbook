module MysqlResources
  module Database
    ...
    class << self
      ...
      def connect(host, username, password, db = 'mysql', connector = 'mysql', socket = nil)
        ...
        conn = MysqlResources::Database.database[connector][host][username] ||= Sequel.connect("#{connector}://#{username}:#{password}@#{host}/#{db}", socket: socket)
